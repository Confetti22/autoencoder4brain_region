...
from lib.utils.load_ckpt import  load_encoder_dict
...
def parse_args():
    ...
    parser.add_argument('-cfg', type =str,help='Configuration file',
                        default='config/contrastive_net.yaml')
...
def train(gpu, args, cfg):
    ...
    # Assuming model.encoder is the part of the model to freeze
    
    #load encoder dict
    load_encoder_dict(model,'testbigger_data6_autoencoder_3layers_64batch')
    ...
    print(model)
    summary(model,(1,*cfg.DATASET.input_size))
    ...
    Trainer(args, cfg,train_loader, model, loss, optimizer).fit()
