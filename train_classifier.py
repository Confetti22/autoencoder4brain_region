...
from lib.utils.load_ckpt import   load_cnn_encoder_dict 
...
def train(gpu, args, cfg):
    ...
    if cfg.MODEL.pretrained:
        ...
        load_cnn_encoder_dict(model,cfg.MODEL.pre_exp_name)
        print(f"encoder_weight dict loaded")
        if not cfg.MODEL.finetune:
            # freeze the parameters of encoder
            print(f"~~~~(not finetune)directly using weights from encoder~~~")
            for param in model.cnn.parameters():
                param.requires_grad = False
        else :
            print(f"~~~~fintune~~~~~")
    else:
        print('###DO NOT USE PRETRAINED MODEL, training from begining')
    ...
    print(model)
    summary(model.cuda(args.gpu),(1,*cfg.DATASET.input_size))
    exit(0)
    ...
    Trainer(args, cfg,train_loader, model, loss, optimizer,valid_loader=valid_loader).fit()
